    ...
    while let Some(item) = stream.next().await {
      ...
      match project_chunk.chunk {
        ...
        Chunk::Semantic(chunk) => {
          ...
          let ordinal = ordinals.get_mut(&file_path).map(|value| {
            let current = *value;
            *value += 1;
            current
          }).unwrap_or(0);
          ...
          self.db.insert_chunk(&record, &embedding)?;
        }
        Chunk::Text(chunk) => {
          let file_path = project_chunk.file_path.clone();
          if !seen_files.contains(&file_path) {
        ...
        Chunk::EndOfFile {
          ...
          if let Some(hash) = content_hash {
            self
              .db
              .upsert_file(&file_path, project_chunk.file_size, hash)?;
          } else {
            return Err(eyre!(
              "missing content hash for {}",
              file_path
            ));
          }
        }
